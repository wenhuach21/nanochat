...
from transformers.masking_utils import create_causal_mask as _orig_create_causal_mask
...
if version.parse(transformers.__version__) < version.parse("4.56.0"):
    import warnings
    warnings.warn(f"transformers {transformers.__version__} < 4.56.0, using compatibility shims")


def create_causal_mask(config, attention_mask, cache_position,
                       input_embeds=None, inputs_embeds=None,
                       past_key_values=None, position_ids=None):
    """自实现的因果 mask，兼容各版本 transformers。"""
    emb = input_embeds if input_embeds is not None else inputs_embeds
    B, T, _ = emb.shape
    device, dtype = emb.device, emb.dtype
    past_len = int(cache_position[0].item()) if cache_position is not None else 0
    total_len = past_len + T
    full_causal = torch.tril(torch.ones(T, total_len, device=device)).bool()
    mask = torch.zeros(B, 1, T, total_len, device=device, dtype=dtype)
    mask = mask.masked_fill(~full_causal, float("-inf"))
    if attention_mask is not None:
        am = attention_mask
        if am.shape[1] < total_len:
            am = torch.cat([
                torch.ones(B, total_len - am.shape[1], device=device, dtype=am.dtype), am
            ], dim=1)
        mask = mask.masked_fill(am[:, None, None, :total_len] == 0, float("-inf"))
    return mask
...
class KimiLinearModel(KimiPreTrainedModel):
    def __init__(self, config: KimiLinearConfig):
        ...
        # 保留用户设置的 attn_implementation（不再强制覆写为 flash_attention_2）
