"""Hyperparameter sweep for the Qwen3.5 (text-only) LLM.

Python port of run_sweep_qwen3p5.sh, modeled on run_sweep.py.

Run from the repository root, e.g.:
    CUDA_VISIBLE_DEVICES=0,1,2,3 python run_sweep_3p5.py
...
# Fixed knobs shared by every run in the sweep.
FIXED_ARGS = {
    "hidden-size": 1024,
    "head-dim": 256,
...
def main():
    ...
    log("=" * 50)
    log(f"Qwen3.5 sweep start (GPUs={os.environ.get('CUDA_VISIBLE_DEVICES')}, nproc={NPROC_PER_NODE})")
    ...
    log("=" * 50)
    log(f"Qwen3.5 sweep complete. Results in: {results_dir}")
    log("=" * 50)
