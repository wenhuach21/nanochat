"""Hyperparameter sweep for the ~0.8B Qwen3.5 (text-only) LLM.

Python port modeled on run_sweep_3p5.py, but the FIXED_ARGS reproduce the
architecture of the official Qwen3.5 0.8B text config, namely:

    hidden_size            = 1024
    head_dim               = 256   (decoupled from hidden_size)
    num_attention_heads    = 8     (NOTE: != hidden_size // head_dim, set explicitly)
    num_key_value_heads    = 2     (GQA)
    intermediate_size      = 3584  (NOTE: != hidden_size * 3, set explicitly)
...
Run from the repository root, e.g.:
    CUDA_VISIBLE_DEVICES=0,1,2,3 python run_sweep_3p5_0p8b.py
...
# Fixed knobs shared by every run in the sweep. These pin the 0.8B architecture.
FIXED_ARGS = {
    "hidden-size": 1024,
    "head-dim": 256,
    "num-attention-heads": 8,     # decoupled from hidden_size // head_dim
    "num-kv-heads": 2,
    "intermediate-size": 3584,    # not hidden_size * 3
...
def main():
    ...
    log("=" * 50)
    log(f"Qwen3.5 0.8B sweep start (GPUs={os.environ.get('CUDA_VISIBLE_DEVICES')}, nproc={NPROC_PER_NODE})")
    ...
    log("=" * 50)
    log(f"Qwen3.5 0.8B sweep complete. Results in: {results_dir}")
    log("=" * 50)
