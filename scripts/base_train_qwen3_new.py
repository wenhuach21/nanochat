...
# Runtime
parser.add_argument("--device-type", type=str, default="cuda", help="cuda|cpu|mps (empty = autodetect)")
# FP8 training
...
# Model architecture
parser.add_argument("--depth", type=int, default=28, help="depth of the Transformer model")
parser.add_argument("--aspect-ratio", type=int, default=36, help="model_dim = depth * aspect_ratio")
...
parser.add_argument("--lr-schedule", type=str, default="defalut", choices=["linear", "cosine", "default"],
                    help="LR decay schedule during warmdown: linear or cosine")
...
def build_model_meta(depth):
    ...
    ## layers= 28
    layers = depth
    hidden_size = (depth * args.aspect_ratio+127)//128*128
    hidden_size=1024
    head_dim = args.head_dim
    hidden_size = (hidden_size+args.head_dim-1)//args.head_dim * args.head_dim
    num_attention_heads = hidden_size // head_dim
    intermediate_size = hidden_size * 3
    config = Qwen3Config(head_dim=head_dim, hidden_act="silu", hidden_size=hidden_size, initializer_range=0.02,
                         intermediate_size=intermediate_size, max_position_embeddings=args.max_seq_len * 10,
                         max_window_layers=layers,
                         num_hidden_layers=layers, num_key_value_heads=num_attention_heads, rms_norm_eps=1e-6,
                         tie_word_embeddings=True, vocab_size=vocab_size, use_cache=False,
...
# )
batch_lr_scale =1.0 # still tuning
...
train_loader = tokenizing_distributed_data_loader_with_state_bos_bestfit(tokenizer, args.device_batch_size,
                                                                         args.max_seq_len, split="train", device=device,
                                                                         resume_state_dict=dataloader_resume_state_dict)
build_val_loader = lambda: tokenizing_distributed_data_loader_bos_bestfit(tokenizer, args.device_batch_size,
                                                                          args.max_seq_len, split="val", device=device)
